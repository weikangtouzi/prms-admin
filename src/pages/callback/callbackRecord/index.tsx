...
import CallbackRecordOperationModal from "@/pages/callback/callbackRecord/CallbackRecordOperationModal";
...
const CallbackRecord = () => {
  ...
  const [current, setCurrent] = useState<
    Partial<CallbackRecordType> | undefined
  >(undefined);
  // 展示弹框
  ...
  const columns: ProColumns<CallbackRecordType>[] = [
    ...
    {
      title: "回访时间",
      dataIndex: "lastTimeCall",
      hideInSearch: true,
      render: (item) => item ? item.slice(0, 10) : "-",
    },
    {
      title: "回访结果",
      dataIndex: "resultOfLastCall",
      hideInSearch: true,
      valueEnum: {
        success: "成功",
        noanswer: "未应答",
        numbernotexist: "号码不存在",
        invalidnumber: "无效号码",
      },
    },
    {
      title: "回访详情",
      dataIndex: "detailOfLastCall",
      hideInSearch: true,
    },
    {
      title: "操作",
  ...
  return (
    <>
      {
        /* <Space>
      <Button type="primary" onClick={() => setCreateVisible(true)}>新增</Button>
    </Space> */
      }
      <ProTable<CallbackRecordType, CallbackRecordFilter>
        ...
        request={async (
          ...
          const msg = await HTAPI.AdminGetCallbackRecordList({
            page: current - 1,
            pageSize: pageSize,
            info,
          });
